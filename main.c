...
void left_rotate(RBPtr self, NodePtr tree, NodePtr x){
    ...
    if ((y->left)->val != NIL)
        (y->left)->parent = x;
    y->parent = x->parent;
    if (x->parent == NULL ||(x->parent)->val == NIL)
        self->root = y;
...
void rb_bh_count(RBPtr self, NodePtr tree, int* bh) {
    if ((tree->right)->val != BLACK)//오른쪽으로
        rb_bh_count(self, tree->right, bh);
...
void rb_transplant(RBPtr self, NodePtr tree, NodePtr u, NodePtr v){
        if (u->parent == NULL || (u->parent)->val == NIL){
            self -> root = v;
            //if ((self->root)->val == NIL)
            //    self -> root = NULL;
...
int rb_find(RBPtr self, NodePtr tree, int data){
    ...
    if ((tree->left)->val != NIL){
        if (rb_find(self, tree->left, data) == 1)
            return 1;
            };

    if ((tree->right)->val != NIL){
        if (rb_find(self, tree->right, data) == 1)
            return 1;
    };
...
void tree_successor(NodePtr x, int searchNum, FILE *search){
    NodePtr y;
    if(x->val == NIL){
        y = x->parent;
        if(searchNum < y->val){
            fprintf(search, "%d ", y->val);
        }else{
    ...
    }else if ((x->right)->val != NIL)
            fprintf(search,"%d ",tree_minimum(x->right)->val);
    else{
    y = x->parent;
    while ( y != NULL && x == y->right){
        x = y;
...
void tree_predecessor(NodePtr x, int searchNum, FILE *search){
    NodePtr y;
    if(x->val == NIL){
        y = x->parent;
        if (searchNum >= y->val){
    ...
    }else if ((x->left)->val != NIL){
        fprintf(search,"%d ", tree_maximum(x->left)->val);
...
void searchList(RBPtr self, NodePtr tree, int searchNum, FILE *search){
    printf("\n<<%d>>\n", searchNum);
    while (searchNum != tree->val){
        if (tree->val == NIL)
            break;
        
        if(searchNum < tree->val){
            if((tree->left)->val == NIL)
    ...
    if (tree->val == NIL){
        fprintf(search, "\n");
        tree_predecessor(tree, searchNum, search);
        fprintf(search,"NIL ");
        tree_successor(tree, searchNum, search);

    }else if(tree->val == searchNum){
        fprintf(search,"\n");
        tree_predecessor(tree, searchNum, search);
        fprintf(search,"%d ", tree->val);
        tree_successor(tree, searchNum, search);
    }
...
int main() {
    ...
    FILE *input = fopen("input/input.txt", "r");// rb input
    FILE *search = fopen("input/search01.txt", "r"); //search input
    FILE *list = fopen("output/list01.txt", "w"); //지우지 못한 값(rb output)
    FILE *searchResult = fopen("output/output.txt", "w"); //search ouput
    DIR *dp;
    struct dirent *ep = NULL;
    dp = opendir ("/Users/haechal/Desktop/ds4/ds4/input/");
    // /Users/haechal/Desktop/ds4/ds4/
    ...
    //search에 해당되는 값을 찾으시오.
    printf("\n***** search progression *****\n");
    while(fscanf(search, "%d", &searchNum) != EOF){
        if (searchNum == 0)
            break;
        else
        searchList(rb, rb->root, searchNum, searchResult);
    }
